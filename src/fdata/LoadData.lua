...
local function setReference(newObj, parent, info, prop)
	local scanForReference = coroutine.wrap(function()
		...
		repeat
			if highestParent.Parent ~= nil and highestParent.Parent:GetAttribute("GUID") then
...
local function checkForObject(parent, info)
	local foundObj = nil
	local aName = "GUID"
	local atName = info.Attributes[aName]
	if atName then
	...
	if not foundObj then
		...
		if not atName then
			info.Attributes[aName] = HttpService:GenerateGUID(false)
		end
	end
	return foundObj
end

local function scanObjects(plr, parent, data, primarypart, objVal)
	--print(data)
	if data then
		for key, info in pairs(data) do
			...
			elseif newObj:IsA("ValueBase") then
				...
				elseif newObj:IsA("ObjectValue") then
					if info.Value then
						scanObjects(plr, newObj, info.Value, nil, true)
					end
			...
			if info.Children then
				scanObjects(plr, newObj, info.Children, isPrimaryPart)
			end

			if objVal then
				newObj.Parent = TempFile
				if parent:IsA("ObjectValue") then
					if parent.Value and parent.Value ~= newObj then
...
function DataSettings:Load(Player, DataFolder, DataTable)
	if DataTable ~= nil then
		scanObjects(Player, Player, DataTable)
